...
NET_CONFIG = {
    # in_channels, kernel_size, downsample, use_rep, use_se, use_shortcut
    "stage1": [64, 3, True, False, True, False, True],
    "stage2": [128, 3, True, False, True, False, True],
    "stage3": [256, 5, False, True, True, True, True],
    "stage4": [512, 5, True, False, True, False, True],
}
...
class SEModule(TheseusLayer):
    def __init__(self, channel, reduction=4, act='relu'):
        ...
        self.conv1 = Conv2D(
            ...
            padding=0)
        #self.act = _create_act(act)
        self.act = nn.PReLU(channel // reduction)
        self.conv2 = Conv2D(
...
class PPLCNetV2(TheseusLayer):
    def __init__(self,
        ...
        for depth_idx, k in enumerate(NET_CONFIG):
            in_channels, kernel_size, downsample, split_pw, use_rep, use_se, use_shortcut = NET_CONFIG[
                k]
            self.stages.append(
                nn.Sequential(* [
        ...
        if self.use_last_conv:
            self.last_conv = Conv2D(
                ...
                bias_attr=False)
            #self.act = _create_act(act)
            self.act = nn.PReLU(self.class_expand)
...
def PPLCNetV2Face_small(pretrained=False, use_ssld=False, **kwargs):
    """
    PPLCNetV2_small
    Args:
...
def PPLCNetV2Face_base(pretrained=False, use_ssld=False, **kwargs):
    """
    PPLCNetV2_base
    Args:
        pretrained: bool=False or str. If `True` load pretrained parameters, `False` otherwise.
...
def PPLCNetV2Face_large(pretrained=False, use_ssld=False, **kwargs):
    """
    PPLCNetV2_large
    Args:
        pretrained: bool=False or str. If `True` load pretrained parameters, `False` otherwise.
